                ...
                return(
                    <div className="relative shadow dark:shadow-gray-700 rounded-md overflow-hidden mt-4 first:mt-0" key={index}>
                        <h2 className="font-medium" id="accordion-collapse-heading-1">
                            <button type="button" onClick={()=>setActiveIndex(item.id)} className={`text-xl text-primary-blue dark:text-white flex justify-between items-center p-5 w-full font-medium text-start ${activeIndex === item.id ? 'bg-gray-50 dark:bg-slate-800' : ''}`}>
